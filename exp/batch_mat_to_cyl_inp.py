#!/usr/bin/env python3
"""Batch cylindrical converter that walks run folders and invokes mat_to_cyl_inp for each manifest."""

"""     how to run:
        python exp/batch_mat_to_cyl_inp.py --roots Matlab/results/stents
"""
...
from mat_to_cyl_inp import convert_cyl_mat_to_inp


def parse_args():
    parser = argparse.ArgumentParser(
        description="Scan manifest files and run mat_to_cyl_inp on each entry (stent meshes)."
    )
    parser.add_argument(
        "--roots",
        nargs="+",
        default=["Matlab/results"],
        help="Root directories to search for mesh_manifest.json files.",
    )
    parser.add_argument(
        "--output-root",
        default=None,
        help="Optional folder where generated INP files are mirrored. Defaults to each run folder.",
    )
    parser.add_argument("--material", default=None, help="Override material name for all runs.")
    parser.add_argument("--density", type=float, default=None, help="Override density value for all runs.")
    parser.add_argument(
        "--elastic",
        type=float,
        nargs=2,
    ...
    parser.add_argument("--peel", type=int, default=0, help="Peel value forwarded to mat_to_cyl_inp (default 0).")
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing .inp files.")
    parser.add_argument("--dry-run", action="store_true", help="Print the actions without writing files.")
    parser.add_argument(
        "--var",
        default=None,
        help="Override variable name inside the MAT file (default: manifest var or solidMaskTiled).",
    )
...
def default_out_name(mask_rel: Path) -> str:
    return f"{mask_rel.stem}_cyl.inp"
...
def main():
    ...
    for manifest_path in manifests:
        ...
        if not mat_path.exists():
            ...
            continue
        var_name = args.var or meta.get("var")
        ...
        out_name = manifest_output or default_out_name(Path(mask_rel))
