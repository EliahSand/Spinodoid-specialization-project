% step3_batch_mat_spline_graph_from_inp
%
% Build MAT/spline control graphs directly from shell INP files.
% This is a side-by-side v4 graph representation; it does not overwrite the
% existing dataset_hybrid schema v3 graphs.
%
% Output:
%   Matlab/GNN/data/dataset_mat_spline/samples/<run_name>/sample.mat
...
if ~exist('datasetRoot', 'var')
    datasetRoot = fullfile(gnnRoot, 'data', 'dataset_mat_spline', 'samples');
end
...
fprintf('Step 3 MAT/spline v4: discovered %d run folders.\n', numel(runDirs));
...
if isempty(taskInpPaths)
    dt = toc(t0);
    fprintf('Step 3 MAT/spline v4 complete: ok=0 skip_no_inp=%d skip_done=%d fail=0 (%.2f min)\n', ...
...
if isfinite(MAX_TASKS) && numel(taskInpPaths) > MAX_TASKS
    ...
    fprintf('Smoke mode: processing first %d MAT/spline graph tasks only.\n', MAX_TASKS);
end

pool = ensure_full_pool();
fprintf('Using parallel pool with %d workers for MAT/spline graph extraction.\n', pool.NumWorkers);
...
nFail = nTasks - nOk;
dt = toc(t0);
fprintf('Step 3 MAT/spline v4 complete: ok=%d skip_no_inp=%d skip_done=%d fail=%d (%.2f min)\n', ...
...
[matSplineGraph, matDebug] = extract_mat_spline_graph_gnn(fullGraph);
...
gnn_data = struct();
gnn_data.edge_index = int32(matSplineGraph.edge_index);
gnn_data.num_nodes = matSplineGraph.num_nodes;
gnn_data.x = matSplineGraph.node_features;
gnn_data.feature_names = matSplineGraph.feature_names;
...
gnn_data.error_tolerance = matSplineGraph.error_tolerance;
gnn_data.reconstruction_error = matSplineGraph.reconstruction_error;
gnn_data.branch_count = matSplineGraph.branch_count;

dense_data = rasterize_full_reference_graph(fullGraph, 128);

save(sampleMatPath, 'gnn_data', 'dense_data', 'matSplineGraph', 'matDebug', '-v7');
end
...
tr_ratio = NaN;
if ~isempty(tk)
    tr_ratio = str2double(tk{1}) / 100;
end
tk = regexp(runName, 'ang(\d+)', 'tokens', 'once');
ang_deg = NaN;
if ~isempty(tk)
    ang_deg = str2double(tk{1});
end
end
