function tile_spinodoid_relief_stent(params)
tStart = tic;
%TILE_SPINODOID_RELIEF_STENT Stack relief stents along Z into one lattice.
%   Generates the same dense-base + spinodal-relief wall as
%   make_spinodoid_relief_stent, then tiles the geometry along the axial
%   direction (like tile_spinodoid_stent) before exporting/logging.
...
if shellThickness <= 0
    error('tile_spinodoid_relief_stent:invalidRadius', ...
        'Outer radius must exceed inner radius.');
end
if cfg.t_base <= 0 || cfg.t_spin <= 0
    error('tile_spinodoid_relief_stent:invalidThickness', ...
...
if abs(layerSum - shellThickness) > tolThickness
    error('tile_spinodoid_relief_stent:thicknessMismatch', ...
...
if nrSpin < 1
    error('tile_spinodoid_relief_stent:spinVoxels', ...
...
addpath(helpersDir);
resultsRoot = fullfile(scriptDir, 'results', 'stent_relief_tiled');
if ~exist(resultsRoot,'dir'), mkdir(resultsRoot); end

%% -------------------- Generate spinodal field (single segment) ---------
rng(cfg.rngSeed);
...
baseLayer = true(cfg.Nt, cfg.Nz, nrBase);
...
runLabelBase = sprintf('stentReliefTiled_%s_Ri%03dum_Ro%03dum_H%03dum_tiles%d', ...
...
stlFileName = sprintf('spinodoid_STENT_RELIEF_TILED_Ri%03dum_Ro%03dum_H%03dum_tiles%d.stl', ...
...
if isempty(fv.vertices)
    error('tile_spinodoid_relief_stent:emptySurface', ...
        'Relief stent surface is empty. Check parameters.');
end
...
stentLines = {
    ...
    sprintf('  Nt/Nz/Nr single: %d / %d / %d', cfg.Nt, cfg.Nz, cfg.Nr)
...
reliefLines = {
    ...
    sprintf('  solid_frac segment: %.3f', solidFractionSegment)
    sprintf('  pattern_coverage_2d: %.3f', solidFractionPattern)
    };

logLines = [
    {
    'Spinodoid relief stent (tiled) run log'
...
% Zero-pad radially so the binary field is enclosed before marching cubes.
