function make_spinodoid_relief_stent(params)
tStart = tic;
%MAKE_SPINODOID_RELIEF_STENT Dense stent wall + spinodoid relief shell.
%   Inner layer: homogeneous stent wall of thickness t_base.
%   Outer layer: spinodoid relief of thickness t_spin derived from the same
%                spectral recipe as the relief sheet, then wrapped with the
%                cylindrical stent pipeline.
...
if shellThickness <= 0
    error('make_spinodoid_relief_stent:invalidRadius', ...
        'Outer radius must exceed inner radius.');
end
if cfg.t_base <= 0 || cfg.t_spin <= 0
    error('make_spinodoid_relief_stent:invalidThickness', ...
...
if abs(layerSum - shellThickness) > tolThickness
    error('make_spinodoid_relief_stent:thicknessMismatch', ...
...
if nrSpin < 1
    error('make_spinodoid_relief_stent:spinVoxels', ...
...
addpath(helpersDir);
resultsRoot = fullfile(scriptDir, 'results', 'stent_relief');
if ~exist(resultsRoot,'dir'), mkdir(resultsRoot); end

%% -------------------- Generate periodic spinodal driver ----------------
rng(cfg.rngSeed);
...
runLabelBase = sprintf('stentRelief_%s_Ri%03dum_Ro%03dum_H%03dum', ...
...
stlFileName = sprintf('spinodoid_STENT_RELIEF_Ri%03dum_Ro%03dum_H%03dum_tspin%03dum.stl', ...
    round(1e6*cfg.Ri), round(1e6*cfg.Ro), round(1e6*cfg.H), round(1e6*cfg.t_spin));
...
if isempty(fv.vertices)
    error('make_spinodoid_relief_stent:emptySurface', ...
...
logLines = [
    {
    'Spinodoid relief stent run log'
...
%PADRADIAL_STENT zero-padding beyond inner/outer radii for watertight STL.
